...
function HouseCard(props: ProjectCardProps): JSX.Element {
  const { photo } = props;
  const [project, setProject] = useState(props.house);
  ...
  const delHouse = (event: ChangeEvent<HTMLInputElement>) => {
    fetch(`http://localhost:3000/lk/delHouse/${project.id}`, {
           method: 'DELETE' ,
           headers: {
...
const editHouse = (event: ChangeEvent<HTMLInputElement>) => {
  fetch(`http://localhost:3000/lk/editHouse/${project.id}`, {
         method: 'GET' ,
         headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        ...
        {/* <CardActionArea> */}
          <Card>
            ...
            <Stack direction="row" spacing={2} margin={1}>
              <Link key={project.id} to={`/editHouse/${project.id}`}>
                <Button onClick={editHouse} variant="contained" sx={{bgcolor: '#fa7301'}} endIcon={<CreateIcon />}>
                  Редактировать
                </Button>
              </Link>
              {/* <Routes>
              ...
              <Button onClick={delHouse} variant="contained" sx={{bgcolor: '#fa7301'}} startIcon={<DeleteIcon />}>
                Удалить
              </Button>
            </Stack>
